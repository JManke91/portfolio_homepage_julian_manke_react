...
import { getFreeImageType, contentfulConfig, getHomeImagesContentType, getGridImageContentType, getWorkCoverImageType, getWorkImageType, getCiteType, getAboutPageType, getGridImagePlacesType, getGridImageThingsType } from '../constants/constants';
...
export async function getPlacesImages() {
  try {
    const response = await contentfulClient.getEntries({
      content_type: getGridImagePlacesType(),
      order: 'sys.createdAt', // Sorting by creation date in descending order
    });
    ...
    console.error('Error fetching places image URLs:', error);
    throw error;
  }
}

export async function getThingsImages() {
  try {
    const response = await contentfulClient.getEntries({
      content_type: getGridImageThingsType(),
      order: 'sys.createdAt', // Sorting by creation date in descending order
    });
    ...
    console.error('Error fetching things image URLs:', error);
    throw error;
  }
}
